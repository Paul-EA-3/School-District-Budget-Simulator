        ...
        <ul className="absolute z-50 w-full bg-white mt-2 rounded-lg shadow-xl border border-slate-100 max-h-[300px] overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
          {predictions.map((prediction, index) => (
            <li
              ...
              className={`px-4 py-3 cursor-pointer flex items-start gap-3 border-b border-slate-50 last:border-0 transition-colors ${index === activeIndex ? 'bg-indigo-50' : 'hover:bg-slate-50'
                }`}
                  ...
                  {placeDetails && !isLoadingDetails && (
                    <div className="mt-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl animate-in fade-in zoom-in-95">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="text-left">
                          <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-tight">Current Selection</div>
                          <div className="font-bold text-slate-800">{placeDetails.name}</div>
                          <div className="text-xs text-slate-500 line-clamp-1">{placeDetails.formatted_address}</div>
                        </div>
                      </div>
                      <button
                        onClick={() => setStep('validate')}
                        className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200 flex items-center justify-center gap-2"
                      >
